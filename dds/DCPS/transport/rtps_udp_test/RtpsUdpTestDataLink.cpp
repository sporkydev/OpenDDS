...
#include "RtpsUdpTestDataLink.h"
#include "RtpsUdpTestTransport.h"
#include "RtpsUdpTestInst.h"
#include "RtpsUdpTestSendStrategy.h"
#include "RtpsUdpTestReceiveStrategy.h"
...
# include "RtpsUdpTestDataLink.inl"
...
RtpsUdpTestDataLink::RtpsUdpTestDataLink(const RtpsUdpTestTransport_rch& transport,
                                 const GuidPrefix_t& local_prefix,
                                 const RtpsUdpTestInst_rch& config,
  ...
  : DataLink(transport, // 3 data link "attributes", below, are unused
    ...
    send_strategy_(make_rch<RtpsUdpTestSendStrategy>(this, config, local_prefix)),
    recv_strategy_(make_rch<RtpsUdpTestReceiveStrategy>(this, local_prefix)),
    ...
    nack_reply_(this, &RtpsUdpTestDataLink::send_nack_replies,
                config->nak_response_delay_),
    heartbeat_reply_(this, &RtpsUdpTestDataLink::send_heartbeat_replies,
                     config->heartbeat_response_delay_),
  heartbeat_(make_rch<HeartBeat>(reactor_task->get_reactor(), reactor_task->get_reactor_owner(), this, &RtpsUdpTestDataLink::send_heartbeats)),
  heartbeatchecker_(make_rch<HeartBeat>(reactor_task->get_reactor(), reactor_task->get_reactor_owner(), this, &RtpsUdpTestDataLink::check_heartbeats)),
...
RtpsUdpTestInst_rch
RtpsUdpTestDataLink::config() const
{
  return config_;
}

bool
RtpsUdpTestDataLink::add_delayed_notification(TransportQueueElement* element)
{
...
void RtpsUdpTestDataLink::do_remove_sample(const RepoId& pub_id,
...
bool
RtpsUdpTestDataLink::open(const ACE_SOCK_Dgram& unicast_socket)
{
  ...
  if (config_->use_multicast_) {
    ...
    if (multicast_socket_.join(config_->multicast_group_address_, 1,
      ...
      ACE_ERROR_RETURN((LM_ERROR,
                        ACE_TEXT("(%P|%t) ERROR: ")
                        ACE_TEXT("RtpsUdpTestDataLink::open: ")
  ...
  if (this->config_->send_buffer_size_ > 0) {
    ...
    if (this->unicast_socket_.set_option(SOL_SOCKET,
      ...
      ACE_ERROR_RETURN((LM_ERROR,
                        ACE_TEXT("(%P|%t) ERROR: ")
                        ACE_TEXT("RtpsUdpTestDataLink::open: failed to set the send buffer size to %d errno %m\n"),
                        snd_size),
                       false);
    }
  }
  ...
  if (this->config_->rcv_buffer_size_ > 0) {
    ...
    if (this->unicast_socket_.set_option(SOL_SOCKET,
      ...
      ACE_ERROR_RETURN((LM_ERROR,
                        ACE_TEXT("(%P|%t) ERROR: ")
                        ACE_TEXT("RtpsUdpTestDataLink::open: failed to set the receive buffer size to %d errno %m \n"),
                        rcv_size),
                       false);
    }
  }
...
void
RtpsUdpTestDataLink::add_locator(const RepoId& remote_id,
...
void
RtpsUdpTestDataLink::get_locators(const RepoId& local_id,
...
ACE_INET_Addr
RtpsUdpTestDataLink::get_locator(const RepoId& remote_id) const
{
  typedef OPENDDS_MAP_CMP(RepoId, RemoteInfo, GUID_tKeyLessThan)::const_iterator iter_t;
  const iter_t iter = locators_.find(remote_id);
  if (iter == locators_.end()) {
    const GuidConverter conv(remote_id);
    ACE_ERROR((LM_ERROR, "(%P|%t) RtpsUdpTestDataLink::get_locator_i() - "
...
void
RtpsUdpTestDataLink::associated(const RepoId& local_id, const RepoId& remote_id,
...
bool
RtpsUdpTestDataLink::check_handshake_complete(const RepoId& local_id,
...
void
RtpsUdpTestDataLink::register_for_reader(const RepoId& writerid,
...
void
RtpsUdpTestDataLink::unregister_for_reader(const RepoId& writerid,
                                       const RepoId& readerid)
{
  ACE_GUARD(ACE_Thread_Mutex, g, lock_);
...
void
RtpsUdpTestDataLink::register_for_writer(const RepoId& readerid,
...
void
RtpsUdpTestDataLink::unregister_for_writer(const RepoId& readerid,
                                       const RepoId& writerid)
{
  ACE_GUARD(ACE_Thread_Mutex, g, lock_);
...
void
RtpsUdpTestDataLink::pre_stop_i()
{
  DBG_ENTRY_LVL("RtpsUdpTestDataLink","pre_stop_i",6);
  DataLink::pre_stop_i();
...
void
RtpsUdpTestDataLink::send_i(TransportQueueElement* element, bool relink)
{
  // Lock here to maintain the locking order:
  // RtpsUdpTestDataLink before RtpsUdpTestSendStrategy
...
RemoveResult
RtpsUdpTestDataLink::remove_sample(const DataSampleElement* sample)
{
  // see comment in RtpsUdpTestDataLink::send_i() for lock order
  ACE_GUARD_RETURN(ACE_Thread_Mutex, g, lock_, REMOVE_ERROR);
  return DataLink::remove_sample(sample);
}

void
RtpsUdpTestDataLink::release_reservations_i(const RepoId& remote_id,
...
void
RtpsUdpTestDataLink::MultiSendBuffer::retain_all(RepoId pub_id)
{
...
void
RtpsUdpTestDataLink::MultiSendBuffer::insert(SequenceNumber /*transport_seq*/,
                                         TransportSendStrategy::QueueType* q,
                                         ACE_Message_Block* chain)
{
  // Called from TransportSendStrategy::send_packet().
  // RtpsUdpTestDataLink is already locked.
  ...
  if (Transport_debug_level > 5) {
    ...
    ACE_DEBUG((LM_DEBUG, "(%P|%t) RtpsUdpTestDataLink::MultiSendBuffer::insert() - "
  ...
  if (tqe->is_fragment()) {
    ...
    if (rce) {
      ...
      const GuidConverter pub(pub_id);
      ACE_ERROR((LM_ERROR, "(%P|%t) RtpsUdpTestDataLink::MultiSendBuffer::insert()"
...
RtpsUdpTestDataLink::customize_queue_element(TransportQueueElement* element)
{
    ...
    if (sub != GUID_UNKNOWN) {
      ...
      if (ri != rw->second.remote_readers_.end()) {
        ...
        if (Transport_debug_level > 3) {
          ...
          ACE_DEBUG((LM_DEBUG,
            "(%P|%t) RtpsUdpTestDataLink::customize_queue_element() - "
...
void
RtpsUdpTestDataLink::end_historic_samples(RtpsWriterMap::iterator writer,
  ...
  if (writer != writers_.end() && writer->second.durable_) {
    ...
    if (sub == GUID_UNKNOWN) {
      if (Transport_debug_level > 3) {
        ...
        ACE_DEBUG((LM_DEBUG, "(%P|%t) RtpsUdpTestDataLink::end_historic_samples "
                   "local %C all readers\n", OPENDDS_STRING(conv).c_str()));
      }
...
bool
RtpsUdpTestDataLink::requires_inline_qos(const PublicationId& pub_id)
{
...
bool RtpsUdpTestDataLink::force_inline_qos_ = false;

void
RtpsUdpTestDataLink::add_gap_submsg(RTPS::SubmessageSeq& msg,
...
void
RtpsUdpTestDataLink::received(const RTPS::DataSubmessage& data,
                          const GuidPrefix_t& src_prefix)
{
  datareader_dispatch(data, src_prefix, &RtpsUdpTestDataLink::process_data_i);
}

bool
RtpsUdpTestDataLink::process_data_i(const RTPS::DataSubmessage& data,
                                const RepoId& src,
  ...
  if (wi != rr.second.remote_writers_.end()) {
    ...
    if (info.recvd_.contains(seq)) {
      if (Transport_debug_level > 5) {
        ...
        ACE_DEBUG((LM_DEBUG, ACE_TEXT("(%P|%t) RtpsUdpTestDataLink::process_data_i(DataSubmessage) -")
...
void
RtpsUdpTestDataLink::deliver_held_data(const RepoId& readerId, WriterInfo& info,
                                   bool durable)
{
...
void
RtpsUdpTestDataLink::received(const RTPS::GapSubmessage& gap,
                          const GuidPrefix_t& src_prefix)
{
  datareader_dispatch(gap, src_prefix, &RtpsUdpTestDataLink::process_gap_i);
}

bool
RtpsUdpTestDataLink::process_gap_i(const RTPS::GapSubmessage& gap,
  ...
  if (wi != rr.second.remote_writers_.end()) {
    ...
    if (sr.first <= sr.second) {
      if (Transport_debug_level > 5) {
        ...
        ACE_DEBUG((LM_DEBUG, "(%P|%t) RtpsUdpTestDataLink::process_gap_i "
      ...
      const GuidConverter conv(src);
      VDBG_LVL((LM_WARNING, "(%P|%t) RtpsUdpTestDataLink::process_gap_i "
...
void
RtpsUdpTestDataLink::received(const RTPS::HeartBeatSubmessage& heartbeat,
                          const GuidPrefix_t& src_prefix)
{
  RepoId src;
  ...
  datareader_dispatch(heartbeat, src_prefix,
                      &RtpsUdpTestDataLink::process_heartbeat_i);
}

bool
RtpsUdpTestDataLink::process_heartbeat_i(const RTPS::HeartBeatSubmessage& heartbeat,
                                     const RepoId& src,
                                     RtpsReaderMap::value_type& rr)
{
  const WriterInfoMap::iterator wi = rr.second.remote_writers_.find(src);
...
bool
RtpsUdpTestDataLink::WriterInfo::should_nack() const
{
...
bool
RtpsUdpTestDataLink::RtpsReader::nack_durable(const WriterInfo& info)
{
...
void
RtpsUdpTestDataLink::send_ack_nacks(RtpsReaderMap::iterator rr, bool finalFlag)
{
  using namespace OpenDDS::RTPS;
    ...
    if (wi->second.ack_pending_ || nack || finalFlag) {
      ...
      if (!locators_.count(wi->first)) {
        if (Transport_debug_level) {
          ...
          ACE_ERROR((LM_ERROR,
                     "(%P|%t) RtpsUdpTestDataLink::send_heartbeat_replies() - "
...
void
RtpsUdpTestDataLink::send_heartbeat_replies() // from DR to DW
{
  using namespace OpenDDS::RTPS;
  ACE_GUARD(ACE_Thread_Mutex, g, lock_);
...
size_t
RtpsUdpTestDataLink::generate_nack_frags(OPENDDS_VECTOR(RTPS::NackFragSubmessage)& nf,
                                     WriterInfo& wi, const RepoId& pub_id)
{
  typedef OPENDDS_MAP(SequenceNumber, RTPS::FragmentNumber_t)::iterator iter_t;
  typedef RtpsUdpTestReceiveStrategy::FragmentInfo::value_type Frag_t;
  RtpsUdpTestReceiveStrategy::FragmentInfo frag_info;
...
void
RtpsUdpTestDataLink::extend_bitmap_range(RTPS::FragmentNumberSet& fnSet,
                                     CORBA::ULong extent)
{
...
void
RtpsUdpTestDataLink::received(const RTPS::HeartBeatFragSubmessage& hb_frag,
                          const GuidPrefix_t& src_prefix)
{
  datareader_dispatch(hb_frag, src_prefix, &RtpsUdpTestDataLink::process_hb_frag_i);
}

bool
RtpsUdpTestDataLink::process_hb_frag_i(const RTPS::HeartBeatFragSubmessage& hb_frag,
                                   const RepoId& src,
                                   RtpsReaderMap::value_type& rr)
{
...
void
RtpsUdpTestDataLink::received(const RTPS::AckNackSubmessage& acknack,
                          const GuidPrefix_t& src_prefix)
{
  // local side is DW
  RepoId local;
  ...
  if (rw == writers_.end()) {
    if (Transport_debug_level > 5) {
      ...
      ACE_DEBUG((LM_WARNING, "(%P|%t) RtpsUdpTestDataLink::received(ACKNACK) "
  ...
  if (Transport_debug_level > 5) {
    ...
    ACE_DEBUG((LM_DEBUG, "(%P|%t) RtpsUdpTestDataLink::received(ACKNACK) "
      "local %C remote %C\n", OPENDDS_STRING(local_conv).c_str(),
  ...
  if (ri == rw->second.remote_readers_.end()) {
    VDBG((LM_WARNING, "(%P|%t) RtpsUdpTestDataLink::received(ACKNACK) "
  ...
  if (!ri->second.durable_data_.empty()) {
    if (Transport_debug_level > 5) {
      ...
      ACE_DEBUG((LM_DEBUG, "RtpsUdpTestDataLink::received(ACKNACK) "
...
void
RtpsUdpTestDataLink::received(const RTPS::NackFragSubmessage& nackfrag,
                          const GuidPrefix_t& src_prefix)
{
  // local side is DW
  RepoId local;
  ...
  if (rw == writers_.end()) {
    if (Transport_debug_level > 5) {
      GuidConverter local_conv(local);
      ACE_DEBUG((LM_WARNING, "(%P|%t) RtpsUdpTestDataLink::received(NACK_FRAG) "
        "WARNING local %C no RtpsWriter\n", OPENDDS_STRING(local_conv).c_str()));
    }
    return;
  }
  ...
  if (Transport_debug_level > 5) {
    GuidConverter local_conv(local), remote_conv(remote);
    ACE_DEBUG((LM_DEBUG, "(%P|%t) RtpsUdpTestDataLink::received(NACK_FRAG) "
      "local %C remote %C\n", OPENDDS_STRING(local_conv).c_str(),
      OPENDDS_STRING(remote_conv).c_str()));
  }
  ...
  const ReaderInfoMap::iterator ri = rw->second.remote_readers_.find(remote);
  if (ri == rw->second.remote_readers_.end()) {
    VDBG((LM_WARNING, "(%P|%t) RtpsUdpTestDataLink::received(NACK_FRAG) "
      "WARNING ReaderInfo not found\n"));
    return;
  }
...
void
RtpsUdpTestDataLink::send_nack_replies()
{
  ACE_GUARD(ACE_Thread_Mutex, g, lock_);
  ...
  for (rw_iter rw = writers_.begin(); rw != writers_.end(); ++rw) {
      ...
      if (ri->second.requested_changes_.size()) {
        if (locators_.count(ri->first)) {
          ...
          if (Transport_debug_level > 5) {
            ...
            ACE_DEBUG((LM_DEBUG, "RtpsUdpTestDataLink::send_nack_replies "
    ...
    if (!requests.empty()) {
      if (writer.send_buff_.is_nil() || writer.send_buff_->empty()) {
        ...
        const RtpsUdpTestSendStrategy::OverrideToken ot =
...
void
RtpsUdpTestDataLink::send_nackfrag_replies(RtpsWriter& writer,
                                       DisjointSequence& gaps,
...
void
RtpsUdpTestDataLink::process_acked_by_all_i(ACE_Guard<ACE_Thread_Mutex>& g, const RepoId& pub_id)
{
  using namespace OpenDDS::RTPS;
  typedef RtpsWriterMap::iterator rw_iter;
  typedef OPENDDS_MULTIMAP(SequenceNumber, TransportQueueElement*)::iterator iter_t;
...
ACE_Message_Block*
RtpsUdpTestDataLink::marshal_gaps(const RepoId& writer, const RepoId& reader,
                              const DisjointSequence& gaps, bool durable)
{
  ...
  if (Transport_debug_level > 5) {
    ...
    ACE_DEBUG((LM_DEBUG, "(%P|%t) RtpsUdpTestDataLink::marshal_gaps "
  ...
  if (durable && reader.entityId == ENTITYID_UNKNOWN) {
    if (Transport_debug_level > 5) {
      ...
      ACE_DEBUG((LM_DEBUG, "RtpsUdpTestDataLink::marshal_gaps local %C "
                 "durable writer\n", OPENDDS_STRING(local_conv).c_str()));
    }
    ...
    for (ReaderInfoMap::iterator ri = rw.remote_readers_.begin();
      ...
      if (!ri->second.expecting_durable_data()) {
        ...
        ACE_DEBUG((LM_DEBUG, "RtpsUdpTestDataLink::marshal_gaps reader "
...
void
RtpsUdpTestDataLink::durability_resend(TransportQueueElement* element)
{
...
void
RtpsUdpTestDataLink::send_durability_gaps(const RepoId& writer,
                                      const RepoId& reader,
...
void
RtpsUdpTestDataLink::send_heartbeats()
{
...

  {
        ...
        if (!ri->second.durable_data_.empty()) {
          ...
          if (now > expiration) {
            ...
            if (Transport_debug_level > 3) {
              ...
              VDBG_LVL((LM_INFO, "(%P|%t) RtpsUdpTestDataLink::send_heartbeats - "
    ...
    if (!subm.empty()) {
      ...
      for (size_t i = 0; i < subm.size(); ++i) {
        if (!(ser << subm[i])) {
          ACE_ERROR((LM_ERROR, "(%P|%t) RtpsUdpTestDataLink::send_heartbeats() - "
...
void
RtpsUdpTestDataLink::check_heartbeats()
{
...
void
RtpsUdpTestDataLink::send_heartbeats_manual(const TransportSendControlElement* tsce)
{
  using namespace OpenDDS::RTPS;
  ...
  if ((ser << hb)) {
    ...
    ACE_ERROR((LM_ERROR, "(%P|%t) RtpsUdpTestDataLink::send_heartbeats_manual() - "
               "failed to serialize HEARTBEAT submessage\n"));
  }
}

RtpsUdpTestDataLink::ReaderInfo::~ReaderInfo()
{
  expire_durable_data();
}

void
RtpsUdpTestDataLink::ReaderInfo::expire_durable_data()
{
  typedef OPENDDS_MAP(SequenceNumber, TransportQueueElement*)::iterator iter_t;
...
bool
RtpsUdpTestDataLink::ReaderInfo::expecting_durable_data() const
{
  return durable_ &&
...
RtpsUdpTestDataLink::RtpsWriter::~RtpsWriter()
{
...
SequenceNumber
RtpsUdpTestDataLink::RtpsWriter::heartbeat_high(const ReaderInfo& ri) const
{
...
void
RtpsUdpTestDataLink::RtpsWriter::add_elem_awaiting_ack(TransportQueueElement* element)
{
...
void
RtpsUdpTestDataLink::TimedDelay::schedule()
{
  if (!scheduled_) {
    const long timer = outer_->get_reactor()->schedule_timer(this, 0, timeout_);

    if (timer == -1) {
      ACE_ERROR((LM_ERROR, "(%P|%t) RtpsUdpTestDataLink::TimedDelay::schedule "
...
void
RtpsUdpTestDataLink::TimedDelay::cancel()
{
  if (scheduled_) {
...
void
RtpsUdpTestDataLink::HeartBeat::enable()
{
  if (!enabled_) {
    ...
    if (timer == -1) {
      ACE_ERROR((LM_ERROR, "(%P|%t) RtpsUdpTestDataLink::HeartBeat::enable"
        " failed to schedule timer %p\n", ACE_TEXT("")));
    } else {
      enabled_ = true;
    }
...
void
RtpsUdpTestDataLink::HeartBeat::disable()
{
  if (enabled_) {
    outer_->get_reactor()->cancel_timer(this);
    enabled_ = false;
...
void
RtpsUdpTestDataLink::send_final_acks (const RepoId& readerid)
{
  ACE_GUARD(ACE_Thread_Mutex, g, lock_);
...
int
RtpsUdpTestDataLink::HeldDataDeliveryHandler::handle_exception(ACE_HANDLE /* fd */)
{
...
void RtpsUdpTestDataLink::HeldDataDeliveryHandler::notify_delivery(const RepoId& readerId, WriterInfo& info)
{
  ACE_ASSERT(link_->reactor_task_->get_reactor_owner() == ACE_Thread::self());
  ...
  for (iter it = info.held_.begin(); it != end; /*increment in loop body*/) {
    if (Transport_debug_level > 5) {
      GuidConverter reader(readerId);
      ACE_DEBUG((LM_DEBUG, ACE_TEXT("(%P|%t) RtpsUdpTestDataLink::HeldDataDeliveryHandler::notify_delivery -")
...
RtpsUdpTestDataLink::HeldDataDeliveryHandler::add_reference()
{
  return link_->add_reference();
}

ACE_Event_Handler::Reference_Count
RtpsUdpTestDataLink::HeldDataDeliveryHandler::remove_reference()
{
