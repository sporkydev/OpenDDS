...
RtpsUdpTestTransport::RtpsUdpTestTransport(const TransportInst_rch& inst)
{
  if (!inst.is_nil()) {
    if (!configure(inst)) {
...
RtpsUdpTestInst_rch
RtpsUdpTestTransport::config() const
{
  return static_rchandle_cast<RtpsUdpTestInst>(TransportImpl::config());
}

RtpsUdpTestDataLink_rch
RtpsUdpTestTransport::make_datalink(const GuidPrefix_t& local_prefix)
{
  RtpsUdpTestDataLink_rch link = make_rch<RtpsUdpTestDataLink>(rchandle_from(this), local_prefix, config(), reactor_task());

  if (!link->open(unicast_socket_)) {
    ACE_ERROR((LM_ERROR,
                      ACE_TEXT("(%P|%t) ERROR: ")
                      ACE_TEXT("RtpsUdpTestTransport::make_datalink: ")
                      ACE_TEXT("failed to open DataLink for socket %d\n"),
                      unicast_socket_.get_handle()));
    return RtpsUdpTestDataLink_rch();
  }

  // RtpsUdpTestDataLink now owns the socket
...
RtpsUdpTestTransport::connect_datalink(const RemoteTransport& remote,
                                   ...
                                   const TransportClient_rch& client )
{
  ...
  RtpsUdpTestDataLink_rch link = link_;
  ...
  if (!link->add_on_start_callback(client, remote.repo_id_)) {
     ...
     VDBG_LVL((LM_DEBUG, "(%P|%t) RtpsUdpTestTransport::connect_datalink got link.\n"), 2);
     return AcceptConnectResult(link);
  }

  GuardType guard(connections_lock_);
  add_pending_connection(client, link);
  VDBG_LVL((LM_DEBUG, "(%P|%t) RtpsUdpTestTransport::connect_datalink pending.\n"), 2);
  return AcceptConnectResult(AcceptConnectResult::ACR_SUCCESS);
}

TransportImpl::AcceptConnectResult
RtpsUdpTestTransport::accept_datalink(const RemoteTransport& remote,
                                  const ConnectionAttribs& attribs,
...
void
RtpsUdpTestTransport::stop_accepting_or_connecting(const TransportClient_rch& client,
...
void
RtpsUdpTestTransport::use_datalink(const RepoId& local_id,
                               const RepoId& remote_id,
...
ACE_INET_Addr
RtpsUdpTestTransport::get_connection_addr(const TransportBLOB& remote,
...
bool
RtpsUdpTestTransport::connection_info_i(TransportLocator& info) const
{
...
void
RtpsUdpTestTransport::register_for_reader(const RepoId& participant,
...
void
RtpsUdpTestTransport::unregister_for_reader(const RepoId& /*participant*/,
                                        const RepoId& writerid,
                                        const RepoId& readerid)
{
  if (link_) {
...
void
RtpsUdpTestTransport::register_for_writer(const RepoId& participant,
                                      const RepoId& readerid,
                                      const RepoId& writerid,
                                      const TransportLocatorSeq& locators,
...
void
RtpsUdpTestTransport::unregister_for_writer(const RepoId& /*participant*/,
                                        const RepoId& readerid,
                                        const RepoId& writerid)
{
  if (link_) {
...
bool
RtpsUdpTestTransport::configure_i(TransportInst* config)
{
  RtpsUdpTestInst* conf = dynamic_cast<RtpsUdpTestInst*>(config);

  if (! conf) {
    ACE_ERROR_RETURN((LM_ERROR,
                      ACE_TEXT("(%P|%t) ERROR: ")
                      ACE_TEXT("RtpsUdpTestTransport::configure_i: ")
  ...
  // unspecified in the RtpsUdpTestInst are known by the time we get to
  ...
  if (!open_appropriate_socket_type(unicast_socket_, conf->local_address())) {
    ACE_ERROR_RETURN((LM_ERROR,
                      ACE_TEXT("(%P|%t) ERROR: ")
                      ACE_TEXT("RtpsUdpTestTransport::configure_i: open_appropriate_socket_type:")
                      ACE_TEXT("%m\n")),
                      false);
  }
  ...
  if (conf->local_address().get_port_number() == 0) {
    ...
    if (unicast_socket_.get_local_addr(address) != 0) {
      ACE_ERROR_RETURN((LM_ERROR,
        ACE_TEXT("(%P|%t) ERROR: RtpsUdpTestDataLink::configure_i - %p\n"),
...
void
RtpsUdpTestTransport::shutdown_i()
{
  if (!link_.is_nil()) {
...
void
RtpsUdpTestTransport::pre_detach(const TransportClient_rch& c)
{
...
bool
RtpsUdpTestTransport::map_ipv4_to_ipv6() const
{
  bool map = false;
  ACE_INET_Addr tmp;
