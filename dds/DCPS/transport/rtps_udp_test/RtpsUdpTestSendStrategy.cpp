...
RtpsUdpTestSendStrategy::RtpsUdpTestSendStrategy(RtpsUdpTestDataLink* link,
...
ssize_t
RtpsUdpTestSendStrategy::send_bytes_i(const iovec iov[], int n)
{
...
RtpsUdpTestSendStrategy::OverrideToken
RtpsUdpTestSendStrategy::override_destinations(const ACE_INET_Addr& destination)
{
...
RtpsUdpTestSendStrategy::OverrideToken
RtpsUdpTestSendStrategy::override_destinations(const OPENDDS_SET(ACE_INET_Addr)& dest)
{
  override_dest_ = &dest;
  return OverrideToken(this);
}

RtpsUdpTestSendStrategy::OverrideToken::~OverrideToken()
{
...
bool
RtpsUdpTestSendStrategy::marshal_transport_header(ACE_Message_Block* mb)
{
...
void
RtpsUdpTestSendStrategy::send_rtps_control(ACE_Message_Block& submessages,
                                       const ACE_INET_Addr& addr)
{
  ...
  if (result < 0) {
    ACE_ERROR((LM_ERROR, "(%P|%t) RtpsUdpTestSendStrategy::send_rtps_control() - "
...
ssize_t
RtpsUdpTestSendStrategy::send_multi_i(const iovec iov[], int n,
                                  const OPENDDS_SET(ACE_INET_Addr)& addrs)
{
  ssize_t result = -1;
...
ssize_t
RtpsUdpTestSendStrategy::send_single_i(const iovec iov[], int n,
                                   const ACE_INET_Addr& addr)
{
  ...
  for (int i = 0; i < n; ++i) {
    if (size_t(iter - buffer + iov[i].iov_len) > UDP_MAX_MESSAGE_SIZE) {
      ACE_ERROR((LM_ERROR, "(%P|%t) RtpsUdpTestSendStrategy::send_single_i() - "
...
void
RtpsUdpTestSendStrategy::add_delayed_notification(TransportQueueElement* element)
{
...
RemoveResult
RtpsUdpTestSendStrategy::do_remove_sample(const RepoId& pub_id,
...
void
RtpsUdpTestSendStrategy::stop_i()
{
}
