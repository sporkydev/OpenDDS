...
RtpsUdpTestReceiveStrategy::RtpsUdpTestReceiveStrategy(RtpsUdpTestDataLink* link, const GuidPrefix_t& local_prefix)
  : link_(link)
  , last_received_()
  , recvd_sample_(0)
...
int
RtpsUdpTestReceiveStrategy::handle_input(ACE_HANDLE fd)
{
  return handle_dds_input(fd);
}

ssize_t
RtpsUdpTestReceiveStrategy::receive_bytes(iovec iov[],
                                      int n,
...
void
RtpsUdpTestReceiveStrategy::deliver_sample(ReceivedDataSample& sample,
    ...
    if (data.readerId != ENTITYID_UNKNOWN) {
      ...
      if (!readers_withheld_.count(reader)) {
        if (Transport_debug_level > 5) {
          ...
          ACE_DEBUG((LM_DEBUG, "(%P|%t) RtpsUdpTestReceiveStrategy[%@]::deliver_sample - calling DataLink::data_received for seq: %q to reader %C\n", this,
      ...
      if (Transport_debug_level > 5) {
        ...
        ACE_DEBUG((LM_DEBUG, "(%P|%t)  - RtpsUdpTestReceiveStrategy[%@]::deliver_sample \nreaders_selected ids:\n%C\n", this, included_ids.c_str()));
        ACE_DEBUG((LM_DEBUG, "(%P|%t)  - RtpsUdpTestReceiveStrategy[%@]::deliver_sample \nreaders_withheld ids:\n%C\n", this, excluded_ids.c_str()));
      }

      if (readers_withheld_.empty() && readers_selected_.empty()) {
        if (Transport_debug_level > 5) {
          ACE_DEBUG((LM_DEBUG, "(%P|%t) RtpsUdpTestReceiveStrategy[%@]::deliver_sample - calling DataLink::data_received for seq: %q TO ALL, no exclusion or inclusion\n", this,
                               sample.header_.sequence_.getValue()));
        }
        link_->data_received(sample);
      } else {
        if (Transport_debug_level > 5) {
          ACE_DEBUG((LM_DEBUG, "(%P|%t) RtpsUdpTestReceiveStrategy[%@]::deliver_sample - calling DataLink::data_received_include for seq: %q to readers_selected_\n", this,
                               sample.header_.sequence_.getValue()));
        }
...
int
RtpsUdpTestReceiveStrategy::start_i()
{
  ACE_Reactor* reactor = link_->get_reactor();
  if (reactor == 0) {
    ACE_ERROR_RETURN((LM_ERROR,
                      ACE_TEXT("(%P|%t) ERROR: ")
                      ACE_TEXT("RtpsUdpTestReceiveStrategy::start_i: ")
...
void
RtpsUdpTestReceiveStrategy::stop_i()
{
  ACE_Reactor* reactor = link_->get_reactor();
  if (reactor == 0) {
    ACE_ERROR((LM_ERROR,
               ACE_TEXT("(%P|%t) ERROR: ")
               ACE_TEXT("RtpsUdpTestReceiveStrategy::stop_i: ")
               ACE_TEXT("NULL reactor reference!\n")));
    return;
  }
...
bool
RtpsUdpTestReceiveStrategy::check_header(const RtpsTransportHeader& header)
{
...
bool
RtpsUdpTestReceiveStrategy::check_header(const RtpsSampleHeader& header)
{
...
RtpsUdpTestReceiveStrategy::withhold_data_from(const RepoId& sub_id)
{
...
void
RtpsUdpTestReceiveStrategy::do_not_withhold_data_from(const RepoId& sub_id)
{
  readers_selected_.insert(sub_id);
}

bool
RtpsUdpTestReceiveStrategy::reassemble(ReceivedDataSample& data)
{
  using namespace RTPS;
  ...
  if (reassembly_.reassemble(frags_, data)) {
    ...
    // in deliver_sample() which ends up calling RtpsUdpTestDataLink::received().
...
bool
RtpsUdpTestReceiveStrategy::remove_frags_from_bitmap(CORBA::Long bitmap[],
...
void
RtpsUdpTestReceiveStrategy::remove_fragments(const SequenceRange& range,
                                         const RepoId& pub_id)
{
...
bool
RtpsUdpTestReceiveStrategy::has_fragments(const SequenceRange& range,
                                      const RepoId& pub_id,
...
RtpsUdpTestReceiveStrategy::MessageReceiver::MessageReceiver(const GuidPrefix_t& local)
...
void
RtpsUdpTestReceiveStrategy::MessageReceiver::reset(const ACE_INET_Addr& addr,
...
void
RtpsUdpTestReceiveStrategy::MessageReceiver::submsg(const RTPS::Submessage& s)
{
  using namespace RTPS;
...
void
RtpsUdpTestReceiveStrategy::MessageReceiver::submsg(
...
void
RtpsUdpTestReceiveStrategy::MessageReceiver::submsg(const RTPS::InfoReplySubmessage& ir)
{
...
void
RtpsUdpTestReceiveStrategy::MessageReceiver::fill_header(
